...
```{r}
(pl_lr_1 + labs(title = "Expected value of y conditioned on x") + theme(plot.title = element_text(size = 15)) + pl_mcmc_1 + labs(title = "Posterior distribution") + theme(plot.title = element_text(size = 15))) / (pl_lr_2  + pl_mcmc_2)
```
\scriptsize	
...
Linear regression implicitly assume equidistant among categories (quality).
...
cumlat_df %>% ggplot(aes(x = value, fill=name)) +
              ...
              scale_fill_manual(
                       ...
                       breaks=c("b_Intercept[1]", "b_Intercept[2]", "b_Intercept[3]", 
                                "b_Intercept[4]", "b_Intercept[5]","b_Intercept[6]"),
                       labels=unname(TeX(c(r"(\tau_4)", r"(\tau_5)", r"(\tau_6)",
                                           r"(\tau_7)", r"(\tau_8)",r"(\tau_9)"))))
```
...
* Cumulative model has lower ELPD.
