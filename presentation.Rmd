...
## Regression: Result
```{r}
mcmc_areas(linear_reg, pars = c("b_alcohol", "b_citric.acid")) + theme(axis.text = element_text(size = 25))
```
...
```{r}
(pl_lr_1 + labs(title = "Expected value") + pl_lr_3 + labs(title = "Posterior predictive")) / (pl_lr_2  + pl_lr_4)
```
...
Consider a latent variable $\tilde{y}$ which determine the 
